...
REDIPS.table = (function() {
    var onmousedown, handler_onmousedown, merge, merge_cells, mergeWithPosition, max_cols, split, get_table, mark, cell_init, row, column, cell_list, relocate, remove_selection, cell_index, cell_ignore, tables = [],
        td_event, show_index, color = {
            cell: false,
            row: false,
            column: false
        },
    ...
    merge = function(mode, clear, table) {
        var tbl, tr, c, rc1, rc2, marked, span, id, cl, t, i, j, first = {
            index: -1,
            span: -1
        };
        remove_selection();
    ...
    mergeWithPosition = function(mode, clear, table , row , col1, col2) {
        var tbl, tr, c, rc1, rc2, marked, span, id, cl, t, i, j, first = {
            index: -1,
            span: -1
        };
        ...
        for (t = 0; t < tbl.length; t++) {
            cl = cell_list(tbl[t]);
            merge_cells(cl,row, col1, col2, mode, clear);
        }
        cell_index();
    };
    ...
    return {
        ...
        mark: mark,
        merge: merge,
        mergeWithPosition: mergeWithPosition,
        split: split,
        row: row,
        column: column,
